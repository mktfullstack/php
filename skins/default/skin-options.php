...
if ( ! function_exists( 'anesta_options_get_list_cpt_options' ) ) {
	function anesta_options_get_list_cpt_options( $cpt, $title = '' ) {
		if ( empty( $title ) ) {
			$title = ucfirst( $cpt );
		}
		return apply_filters( 'anesta_filter_get_list_cpt_options',
								array_merge(
									anesta_options_get_list_cpt_options_body( $cpt, $title ),              // Body style options for both: a posts list and a single post
