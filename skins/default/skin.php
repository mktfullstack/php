...
if ( ! function_exists( 'anesta_skin_media_for_load_css_responsive' ) ) {
	...
	function anesta_skin_media_for_load_css_responsive( $media, $slug ) {
		if ( in_array( $slug, array( 'main', 'single', 'gutenberg-general', 'bbpress', 'elementor', 'trx-addons', 'blog-styles', 'buddypress-docs', 'bp-better-messages', 'wp-job-manager', 'sfwd-lms' ) ) ) {
			$media = 'xxl';
		} else if ( in_array( $slug, array( 'front-page', 'tribe-events', 'trx-addons-layouts', 'woocommerce', 'wp-job-manager-resumes' ) ) ) {
			$media = 'xl';
		} else if ( in_array( $slug, array( 'edd', 'mptt', 'woocommerce-extensions', 'single-styles', 'democracy-poll', 'echo-knowledge-base' ) ) ) {
			$media = 'lg';
		} else if ( in_array( $slug, array( 'vc', 'learnpress', 'theme-hovers', 'paid-memberships-pro' ) ) ) {
			$media = 'md';
		} else if ( in_array( $slug, array( 'instagram-feed' ) ) ) {
			$media = 'sm';
		} else if ( in_array( $slug, array( 'booked', 'gutenberg' ) ) ) {
			$media = 'xs';
		}
		return $media;
	}
		...
		return anesta_array_merge(
			array(
				...
				'colors_text_hover'  => array(
					...
					'std'      => '',
					'val'      => anesta_get_scheme_color( 'accent_hover' ),
					'qsetup'   => esc_html__( 'General', 'anesta' ),
					'type'     => 'color',
				),
				...
				'colors_text_hover2' => array(
					...
					'std'      => '',
					'val'      => anesta_get_scheme_color( 'accent_hover2' ),
					'qsetup'   => esc_html__( 'General', 'anesta' ),
					'type'     => 'color',
				),
				...
				'colors_text_hover3' => array(
					...
					'std'      => '',
					'val'      => anesta_get_scheme_color( 'accent_hover3' ),
					'qsetup'   => esc_html__( 'General', 'anesta' ),
					'type'     => 'color',
				),
			),
