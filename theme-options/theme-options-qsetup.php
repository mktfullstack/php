			...
			<div class="anesta_options anesta_options_qsetup">
				<form action="<?php echo esc_url( get_admin_url( null, 'admin.php?page=trx_addons_theme_panel' ) ); ?>" class="trx_addons_theme_panel_section_form" name="trx_addons_theme_panel_qsetup_form" method="post">
					...
					<div class="anesta_options_buttons trx_buttons">
						<input type="button" class="anesta_options_button_submit trx_addons_button trx_addons_button_accent" value="<?php esc_attr_e( 'Save Options', 'anesta' ); ?>">
					</div>
				</form>
			</div>
			<?php
...
if ( ! function_exists( 'anesta_options_qsetup_save_options' ) ) {
	...
	function anesta_options_qsetup_save_options() {
		...
		if ( ! empty( $scheme_storage ) ) {
			...
			if ( is_array( $schemes ) ) {
				$color_scheme = get_theme_mod( anesta_storage_get_array( 'schemes_sorted', 0 ) );
