...
jQuery( document ).ready(
	function() {
		...
		anesta_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="anesta_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.anesta_post_editor' );
						if ( link.length > 0 ) {
							anesta_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.anesta_post_editor', function(e) {
						anesta_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('anesta_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function anesta_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
