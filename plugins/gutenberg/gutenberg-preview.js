...
jQuery( window ).on( 'load', function() {
	...
	if ( typeof window.MutationObserver !== 'undefined' ) {
		...
		if ( $editor_wrapper.length ) {
			...
			anesta_create_observer( 'check_visual_editor_wrapper', $editor_wrapper, function( mutationsList ) {
				var $editor = anesta_gutenberg_editor_object();
				if ( $editor.length ) {
					anesta_gutenberg_first_init( $editor );
				}
			} );
			...
			if ( $editor_wrapper.attr( 'id' ) == 'widgets-editor' ) {
				anesta_create_observer( 'check_editor_styles_wrapper', $editor_wrapper, function( mutationsList ) {
					var $styles_wrapper = $editor_wrapper.find( '.editor-styles-wrapper:not([class*="scheme_"])' );
					if ( $styles_wrapper.length ) {
						$styles_wrapper.addClass( 'scheme_' + ANESTA_STORAGE['color_scheme'] );
					} else {
						anesta_remove_observer( 'check_editor_styles_wrapper' );
					}
				} );
			}
		}
	...
	function anesta_gutenberg_editor_object() {
		// Get Post Editor
		var $editor = jQuery( '.edit-post-visual-editor:not(.anesta_inited)' ).eq( 0 );
		if ( ! $editor.length ) {
			...
			if ( $editor_frame.length ) {
				...
				if ( $editor_frame.hasClass('editor-styles-wrapper') && ! $editor_frame.hasClass('anesta_inited') ) {
					$editor = $editor_frame;
				}
			} else {
				// Check if Widgets Editor exists
				$editor = jQuery( '.edit-widgets-block-editor:not(.anesta_inited)' ).eq( 0 );
			}
		}
		return $editor;
	}
	...
	// Init on page load
	function anesta_gutenberg_first_init( $editor ) {

		// Get Gutenberg editor object
		if ( ! $editor ) {
			$editor = anesta_gutenberg_editor_object();
			if ( ! $editor.length ) {
				return;
			}
		}
		...
		styles_wrapper.addClass( 'scheme_' + ANESTA_STORAGE['color_scheme'] );
		writing_flow.addClass( 'scheme_' + ANESTA_STORAGE['color_scheme'] );

		if ( ! widgets_GB ) {
			if ( ! fse_GB ) {
				// Copy post-type to the styles_wrapper
				styles_wrapper.addClass( anesta_get_class_by_prefix( $editor.attr('class'), 'post-type-' ) );
				// Decorate sidebar placeholder
				styles_wrapper
					.addClass( 'sidebar_position_' + ANESTA_STORAGE['sidebar_position'] )
					.addClass( ANESTA_STORAGE['expand_content'] + '_content' );
				if ( ANESTA_STORAGE['sidebar_position'] == 'left' && old_GB ) {
					sidebar_wrapper.prepend( '<div class="editor-post-sidebar-holder"></div>' );
				} else if ( ANESTA_STORAGE['sidebar_position'] != 'hide' ) {
