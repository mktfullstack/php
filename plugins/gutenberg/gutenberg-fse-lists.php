...
if ( ! function_exists( 'anesta_gutenberg_fse_list_header_styles' ) ) {
	add_filter( 'anesta_filter_list_header_styles', 'anesta_gutenberg_fse_list_header_styles');
	/**
	 * Add a WordPress FSE templates to the headers list.
	 * 
	 * Hooks: add_filter( 'anesta_filter_list_header_styles', 'anesta_gutenberg_fse_list_header_styles');
	 * 
	 * @param array $list  Optional. An array with a list of headers from other available Page Builders.
	 */
	function anesta_gutenberg_fse_list_header_styles( $list = array() ) {
		$new_list = array();
		// Add a default templates
		$data = anesta_gutenberg_fse_theme_json_data();
		...
		$layouts = anesta_get_list_posts(
			false, array(
				'post_type'    => ANESTA_FSE_TEMPLATE_PART_PT,
				'orderby'      => 'ID',
				'order'        => 'asc',
				'not_selected' => false,
				//'return'       => 'post_name',
...
if ( ! function_exists( 'anesta_gutenberg_fse_list_footer_styles' ) ) {
	add_filter( 'anesta_filter_list_footer_styles', 'anesta_gutenberg_fse_list_footer_styles');
	/**
	 * Add a WordPress FSE templates to the footers list.
	 * 
	 * Hooks: add_filter( 'anesta_filter_list_footer_styles', 'anesta_gutenberg_fse_list_footer_styles');
	 * 
	 * @param array $list  Optional. An array with a list of footers from other available Page Builders.
	 */
	function anesta_gutenberg_fse_list_footer_styles( $list = array() ) {
		$new_list = array();
		// Add a default templates
		$data = anesta_gutenberg_fse_theme_json_data();
		if ( ! empty( $data['templateParts'] ) && is_array( $data['templateParts'] ) ) {
