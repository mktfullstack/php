...
if ( ! function_exists( 'anesta_gutenberg_fse_replace_frontpage_content' ) ) {
	add_filter( 'frontpage_template', 'anesta_gutenberg_fse_replace_frontpage_content', 9999, 3 );
	/**
	 ...
	 * 
	 * Hooks: add_filter( 'frontpage_template', 'anesta_gutenberg_fse_replace_frontpage_content', 9999, 3 );
	 *
	 * @param string   $template  Path to the template. See locate_template().
	 * @param string   $type      Sanitized filename without extension.
	 * @param string[] $templates A list of template candidates, in descending order of priority.
	 */
	function anesta_gutenberg_fse_replace_frontpage_content( $template, $type = '', $templates = array() ) {
		if ( substr( $template, -19 ) == 'template-canvas.php'
			&& get_option( 'show_on_front' ) == 'page'
			&& anesta_is_on( anesta_get_theme_option( 'front_page_enabled', false ) )
		) {
			if ( have_posts() ) {
				the_post();
			}
...
if ( ! function_exists( 'anesta_gutenberg_fse_modify_template_with_current_options' ) ) {
	foreach( anesta_get_wp_template_hooks() as $hook ) {
		add_filter( $hook, 'anesta_gutenberg_fse_modify_template_with_current_options', 30, 3 );
	}
	/**
	 ...
	 * 
	 * Trigger the filter 'anesta_filter_wp_current_template_content' to allow other modules
	 ...
	 */
	function anesta_gutenberg_fse_modify_template_with_current_options( $template, $type = '', $templates = array() ) {
		global $_wp_current_template_content;
		if ( substr( $template, -19 ) == 'template-canvas.php' && is_array( $templates ) ) {
			$_wp_current_template_content = apply_filters( 'anesta_filter_wp_current_template_content', $_wp_current_template_content, $template, $type, $templates );
		}
		return $template;
	}
}
...
// Header
//----------------------------------------------
if ( ! function_exists( 'anesta_gutenberg_fse_modify_template_replace_header' ) ) {
	add_filter( 'anesta_filter_wp_current_template_content', 'anesta_gutenberg_fse_modify_template_replace_header', 10, 4 );
	/**
	 * Replace a header in the content of the current template
	 * according to the Theme Options of the current page.
	 * 
	 * Hooks: add_filter( 'anesta_filter_wp_current_template_content', 'anesta_gutenberg_fse_modify_template_replace_header', 10, 4 );
	 *
	 ...
	 * @param array  $templates  A list of template candidates, in descending order of priority.
	 */
	function anesta_gutenberg_fse_modify_template_replace_header( $content, $template, $type, $templates ) {
		$header_type = anesta_get_theme_option( 'header_type' );
		if ( 'custom' == $header_type ) {
			$header_style = anesta_get_theme_option( "header_style" );
			...
			} else if ( anesta_is_layouts_available() ) {
				ob_start();
				// Trigger action before the custom header to allow other modules include an additional content to the custom header
				do_action( 'anesta_action_fse_before_custom_header', $content, $template, $type, $templates );
				// Custom header
				get_template_part( apply_filters( 'anesta_filter_get_template_part', "templates/header-custom" ) );
				// Trigger action after th custom header to allow other modules include an additional content to the custom header
				do_action( 'anesta_action_fse_after_custom_header', $content, $template, $type, $templates );
				// Get output
...
if ( ! function_exists( 'anesta_gutenberg_fse_custom_header_add_side_menu' ) ) {
	add_action( 'anesta_action_fse_after_custom_header', 'anesta_gutenberg_fse_custom_header_add_side_menu', 10, 4 );
	/**
	 * Add a side menu to the custom header layout.
	 * 
	 * Hooks: add_action( 'anesta_action_fse_after_custom_header', 'anesta_gutenberg_fse_custom_header_add_side_menu', 10, 4 );
	 *
	 * @param string $content    A content of the current template.
	 * @param string $template   Path to the template. See locate_template().
	 * @param string $type       Sanitized filename without extension.
	 * @param array  $templates  A list of template candidates, in descending order of priority.
	 */
	function anesta_gutenberg_fse_custom_header_add_side_menu( $content, $template, $type, $templates ) {
		// Side menu
...
if ( ! function_exists( 'anesta_gutenberg_fse_custom_header_add_mobile_menu' ) ) {
	add_action( 'anesta_action_fse_after_custom_header', 'anesta_gutenberg_fse_custom_header_add_mobile_menu', 10, 4 );
	/**
	 * Add a mobile menu to the custom header layout.
	 * 
	 * Hooks: add_action( 'anesta_action_fse_after_custom_header', 'anesta_gutenberg_fse_custom_header_add_mobile_menu', 10, 4 );
	 *
	 * @param string $content    A content of the current template.
	 * @param string $template   Path to the template. See locate_template().
	 * @param string $type       Sanitized filename without extension.
	 * @param array  $templates  A list of template candidates, in descending order of priority.
	 */
	function anesta_gutenberg_fse_custom_header_add_mobile_menu( $content, $template, $type, $templates ) {
		// Mobile menu
...
// Sidebar
//----------------------------------------------
if ( ! function_exists( 'anesta_gutenberg_fse_modify_template_replace_sidebar' ) ) {
	add_filter( 'anesta_filter_wp_current_template_content', 'anesta_gutenberg_fse_modify_template_replace_sidebar', 10, 4 );
	/**
	 ...
	 * 
	 * Hooks: add_filter( 'anesta_filter_wp_current_template_content', 'anesta_gutenberg_fse_modify_template_replace_sidebar', 10, 4 );
	 *
	 * @param string $content    A content of the current template.
	 * @param string $template   Path to the template. See locate_template().
	 * @param string $type       Sanitized filename without extension.
	 * @param array  $templates  A list of template candidates, in descending order of priority.
	 */
	function anesta_gutenberg_fse_modify_template_replace_sidebar( $content, $template, $type, $templates ) {
		// Replace sidebar
		if ( anesta_sidebar_present() ) {
			$sidebar_type = anesta_get_theme_option( 'sidebar_type' );
			if ( 'custom' == $sidebar_type && ! anesta_is_layouts_available() ) {
				$sidebar_type = 'default';
			}
			// Masks to search a sidebar block in the content
			$sidebar_start = apply_filters( 'anesta_filter_wp_block_with_sidebar_start',
											'(<!-- wp:group[\s]*{[^}]*"className":[\s]*"[^"]*sidebar[^"]*"[^>]*-->[\s]*'
											. '<div[^>]*class="[^"]*sidebar[^"]*"[^>]*>)'
											);
			$sidebar_end = apply_filters( 'anesta_filter_wp_block_with_sidebar_end',
											'(</div>[\s]*'
											. '<!-- /wp:group -->)'
											);
			...
			// A custom sidebar (built with FSE or with any other builder)
				...
				$sidebar_style = anesta_get_theme_option( "sidebar_style" );
				$sidebar_id = anesta_get_custom_sidebar_id();
				...
				// Custom sidebar's layout
					...
					do_action( 'anesta_action_fse_before_custom_sidebar', $content, $template, $type, $templates );
					// Custom sidebar
					...
					do_action( 'anesta_action_fse_after_custom_sidebar', $content, $template, $type, $templates );
					// Get output
					$html = ob_get_contents();
					ob_end_clean();
					if ( ! empty( $html ) ) {
...
// Footer
//----------------------------------------------
if ( ! function_exists( 'anesta_gutenberg_fse_modify_template_replace_footer' ) ) {
	add_filter( 'anesta_filter_wp_current_template_content', 'anesta_gutenberg_fse_modify_template_replace_footer', 10, 4 );
	/**
	 * Replace a footer in the content of the current template
	 * according to the Theme Options of the current page.
	 * 
	 * Hooks: add_filter( 'anesta_filter_wp_current_template_content', 'anesta_gutenberg_fse_modify_template_replace_footer', 10, 4 );
	 *
	 * @param string $content    A content of the current template.
	 * @param string $template   Path to the template. See locate_template().
	 * @param string $type       Sanitized filename without extension.
	 * @param array  $templates  A list of template candidates, in descending order of priority.
	 */
	function anesta_gutenberg_fse_modify_template_replace_footer( $content, $template, $type, $templates ) {
		$footer_type = anesta_get_theme_option( 'footer_type' );
		if ( 'custom' == $footer_type ) {
			$footer_style = anesta_get_theme_option( "footer_style" );
			...
			// Custom footer's layout
				...
				do_action( 'anesta_action_fse_before_custom_footer', $content, $template, $type, $templates );
				// Custom footer
				get_template_part( apply_filters( 'anesta_filter_get_template_part', "templates/footer-custom" ) );
				// Trigger action after th custom footer to allow other modules include an additional content to the custom footer
				do_action( 'anesta_action_fse_after_custom_footer', $content, $template, $type, $templates );
				// Get output
				$html = ob_get_contents();
				ob_end_clean();
				if ( ! empty( $html ) ) {
...
//------------------------------------------------------
if ( ! function_exists( 'anesta_gutenberg_fse_replace_featured_image_renderer' ) ) {
	add_filter( 'block_type_metadata_settings', 'anesta_gutenberg_fse_replace_featured_image_renderer', 10, 2 );
	/**
	 ...
	 */
	function anesta_gutenberg_fse_replace_featured_image_renderer( $settings = array(), $metadata = array() ) {
		if ( ! empty( $settings['render_callback'] ) && $settings['render_callback'] == 'render_block_core_post_featured_image' ) {
			$settings['render_callback'] = 'anesta_gutenberg_fse_featured_image_renderer';
		}
		return $settings;
	}
}

if ( ! function_exists( 'anesta_gutenberg_fse_featured_image_renderer' ) ) {
	/**
	 ...
	 */
	function anesta_gutenberg_fse_featured_image_renderer( $attributes, $content, $block ) {
		...
		$anesta_hover      = anesta_get_theme_option( 'image_hover' );
		$anesta_components = anesta_array_get_keys_by_value( anesta_get_theme_option( 'meta_parts' ) );
		...
		anesta_show_post_featured( apply_filters( 'anesta_filter_args_featured',
			array(
				...
				'singular'   => anesta_is_singular(),
				'thumb_size' => anesta_get_thumb_size( strpos( anesta_get_theme_option( 'body_style' ), 'full' ) !== false
									? 'full'
									: ( $anesta_expanded || anesta_is_singular()
										? 'huge' 
										: 'big' 
										)
									),
