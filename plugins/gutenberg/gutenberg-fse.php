...
if ( ! defined( 'ANESTA_FSE_TEMPLATE_PART_PT' ) ) define( 'ANESTA_FSE_TEMPLATE_PART_PT', 'wp_template_part' );

// Load additional files with FSE support
require_once anesta_get_file_dir( 'plugins/gutenberg/gutenberg-fse-lists.php' );
require_once anesta_get_file_dir( 'plugins/gutenberg/gutenberg-fse-templates.php' );


if ( ! function_exists( 'anesta_gutenberg_fse_loader' ) ) {
	add_action( 'wp_loaded', 'anesta_gutenberg_fse_loader', 1 );
	/**
	 * Turn off a Gutenberg (plugin) templates on frontend
	 * if a theme-specific frontend editor is active.
	 */
	function anesta_gutenberg_fse_loader() {
		// 
		if ( anesta_exists_gutenberg_fse()
			&& get_option( 'show_on_front' ) == 'page'
			&& (int) get_option('page_on_front') > 0
			&& anesta_is_on( anesta_get_theme_option( 'front_page_enabled', false ) )
			&& anesta_get_current_url() == '/'
		) {
...
if ( ! function_exists( 'anesta_gutenberg_is_fse_wp_support' ) ) {
	/**
	 * Check if WordPress with FSE support installed
	 * (WordPress 5.9+ Site Editor is present).
	 */
	function anesta_gutenberg_is_fse_wp_support() {
		return function_exists( 'wp_is_block_theme' );
	}
}

if ( ! function_exists( 'anesta_gutenberg_is_fse_gb_support' ) ) {
	/**
	 * Check if a plugin "Gutenberg" with FSE support is installed.
	 */
	function anesta_gutenberg_is_fse_gb_support() {
		return function_exists( 'gutenberg_is_fse_theme' );
	}
}

if ( ! function_exists( 'anesta_exists_gutenberg_fse' ) ) {
	/**
	 * Check if FSE is available
	 * (WordPress 5.9+ or the plugin "Gutenberg" are installed).
	 */
	function anesta_exists_gutenberg_fse() {
		return anesta_exists_gutenberg() && ( anesta_gutenberg_is_fse_wp_support() || anesta_gutenberg_is_fse_gb_support() );
	}
}

if ( ! function_exists( 'anesta_gutenberg_is_fse_theme' ) ) {
	/**
	 * Check if the current theme supports a FSE behaviour.
	 */
	function anesta_gutenberg_is_fse_theme() {
		static $fse = -1;
		if ( $fse == -1 ) {
			if ( anesta_gutenberg_is_fse_wp_support() ) {                                    // WordPress 5.9+ FSE
				$fse = (bool) wp_is_block_theme();
			} else if ( anesta_gutenberg_is_fse_gb_support() ) {                             // Plugin Gutenberg FSE
				$fse = (bool) gutenberg_is_fse_theme();
			} else {
				$fse = is_readable( ANESTA_THEME_DIR . 'templates/index.html' )              // WordPress 5.9+ FSE
						|| ( ANESTA_THEME_DIR != ANESTA_CHILD_DIR && is_readable( ANESTA_CHILD_DIR . 'templates/index.html' ) )
						|| is_readable( ANESTA_THEME_DIR . 'block-templates/index.html' )    // Plugin Gutenberg FSE
						|| ( ANESTA_THEME_DIR != ANESTA_CHILD_DIR && is_readable( ANESTA_CHILD_DIR . 'block-templates/index.html' ) );
			}
		}
		return $fse;
	}
...
if ( ! function_exists( 'anesta_gutenberg_is_fse_enabled' ) ) {
	/**
	 * Check if the current theme supports FSE behaviour and FSE is installed and activated
	 */
	function anesta_gutenberg_is_fse_enabled() {
		return anesta_exists_gutenberg_fse() && anesta_gutenberg_is_fse_theme();
	}
}

if ( ! function_exists( 'anesta_gutenberg_fse_copy_from_skin' ) ) {
	add_action( 'anesta_action_skin_switched', 'anesta_gutenberg_fse_copy_from_skin', 10, 2 );
	/**
	 * Copy FSE-required folders from a new skin subfolder '_fse' to the theme's root folder.
	 * 
	 * Hooks: add_action( 'anesta_action_skin_switched', 'anesta_gutenberg_fse_copy_from_skin', 10, 2 );
	 */
	function anesta_gutenberg_fse_copy_from_skin( $new_skin, $old_skin ) {
		$theme_templates_dir = anesta_prepare_path( ANESTA_THEME_DIR . ( anesta_gutenberg_is_fse_wp_support() ? 'templates/' : 'block-templates/' ) );
		$theme_template_parts_dir = anesta_prepare_path( ANESTA_THEME_DIR . ( anesta_gutenberg_is_fse_wp_support() ? 'parts/' : 'block-template-parts/' ) );
		$theme_json = anesta_prepare_path( ANESTA_THEME_DIR . 'theme.json' );
		$skin_templates_dir = anesta_prepare_path( ANESTA_THEME_DIR . 'skins/' . $new_skin . '/_fse' . ( anesta_gutenberg_is_fse_wp_support() ? '/templates/' : '/block-templates/' ) );
		$skin_template_parts_dir = anesta_prepare_path( ANESTA_THEME_DIR . 'skins/' . $new_skin . '/_fse' . ( anesta_gutenberg_is_fse_wp_support() ? '/parts/' : '/block-template-parts/' ) );
		// Remove old templates from the stylesheet dir (if exists)
		if ( is_dir( $theme_templates_dir ) ) {
			anesta_unlink( $theme_templates_dir );
		}
		if ( is_dir( $theme_template_parts_dir ) ) {
			anesta_unlink( $theme_template_parts_dir );
		}
		if ( file_exists( $theme_json ) ) {
			anesta_unlink( $theme_json );
		}
		...
		if ( is_dir( $skin_templates_dir ) ) {
			anesta_copy( $skin_templates_dir, $theme_templates_dir );
		}
		if ( is_dir( $skin_template_parts_dir ) ) {
			anesta_copy( $skin_template_parts_dir, $theme_template_parts_dir );
		}
	}
}

if ( ! function_exists( 'anesta_gutenberg_fse_update_theme_json' ) ) {
	add_action( 'anesta_action_skin_switched', 'anesta_gutenberg_fse_update_theme_json', 30, 2 );
	add_action( 'anesta_action_save_options', 'anesta_gutenberg_fse_update_theme_json', 30 );
	add_action( 'trx_addons_action_save_options', 'anesta_gutenberg_fse_update_theme_json', 30 );
	/**
	 * Update the file 'theme.json' after the current skin is switched and/or theme options are saved.
	 * 
	 * Hooks: add_action( 'anesta_action_skin_switched', 'anesta_gutenberg_fse_update_theme_json', 30, 2 );
	 *        add_action( 'anesta_action_save_options', 'anesta_gutenberg_fse_update_theme_json', 30 );
	 *        add_action( 'trx_addons_action_save_options', 'anesta_gutenberg_fse_update_theme_json', 30 );
	 * 
	 * Trigger filter 'anesta_filter_gutenberg_fse_theme_json_data' to allow other modules modify this data before saving.
	 */
	function anesta_gutenberg_fse_update_theme_json( $new_skin = '', $old_skin = '' ) {
		// If the current skin supports FSE
		if ( is_dir( ANESTA_THEME_DIR . anesta_skins_get_current_skin_dir( $new_skin ) . '_fse' ) ) {
			$theme_json_data = anesta_gutenberg_fse_theme_json_data();
			if ( ! empty( $theme_json_data ) && is_array( $theme_json_data ) ) {
				anesta_fpc( anesta_prepare_path( ANESTA_THEME_DIR . 'theme.json' ), json_encode( $theme_json_data, JSON_PRETTY_PRINT | JSON_UNESCAPED_SLASHES ) );
			}
		}
	}
}

if ( ! function_exists( 'anesta_gutenberg_fse_theme_json_data' ) ) {
	/**
	 * Return a default data to save to the file 'theme.json'
	 * 
	 * Trigger filter 'anesta_filter_gutenberg_fse_theme_json_data' to allow other modules change data.
	 * 
	 * @return array  Data for the 'theme.json'
	 */
	function anesta_gutenberg_fse_theme_json_data() {
		...
		$data = array(
					...
					"customTemplates" => array(
						array(
							"name"      => "blog",
							"title"     => esc_html__( "Blog archive", 'anesta' ),
							"postTypes" => array(
								"page"
							)
						)
					...
					"templateParts" => array(
						array(
							"name"  => "header",
							"title" => esc_html__( "Header for FSE", 'anesta' ),
							"area"  => "header",
						),
						array(
							"name"  => "sidebar",
							"title" => esc_html__( "Sidebar for FSE", 'anesta' ),
							"area"  => "uncategorized",
						),
						array(
							"name"  => "content-404",
							"title" => esc_html__( "Page 404 content", 'anesta' ),
							"area"  => "uncategorized",
						),
						array(
							"name"  => "blog-post-standard",
							"title" => esc_html__( "Blog post item: Standard", 'anesta' ),
							"area"  => "uncategorized",
						),
						array(
							"name"  => "blog-post-header",
							"title" => esc_html__( "Blog post header", 'anesta' ),
							"area"  => "uncategorized",
						),
						array(
							"name"  => "blog-pagination",
							"title" => esc_html__( "Blog pagination", 'anesta' ),
							"area"  => "uncategorized",
						),
						array(
							"name"  => "footer",
							"title" => esc_html__( "Footer for FSE", 'anesta' ),
							"area"  => "footer",
						),
					),
...
*/
		...
		$data['settings']['color']['duotone'][] = array(
			...
			"slug"   => 'bg-and-text',
			"name"   => esc_html__( 'Background and text color', 'anesta' )
		);
		$data['settings']['color']['duotone'][] = array(
			"colors" => array( $scheme['bg_color'], $scheme['text_dark'] ),
			"slug"   => 'bg-and-dark',
			"name"   => esc_html__( 'Background and dark color', 'anesta' )
		);
		$data['settings']['color']['duotone'][] = array(
			"colors" => array( $scheme['bg_color'], $scheme['text_link'] ),
			"slug"   => 'bg-and-link',
			"name"   => esc_html__( 'Background and link color', 'anesta' )
		);
		...
		$data['settings']['color']['gradients'][] = array(
			...
			"name"     => esc_html__( 'Vertical from link color to hover color', 'anesta' ),
		);
		$data['settings']['color']['gradients'][] = array(
			...
			"name"     => esc_html__( 'Diagonal from link color to hover color', 'anesta' ),
		);
/*
		// Add fonts
...
*/
		...
		return apply_filters( 'anesta_filter_gutenberg_fse_theme_json_data', $data );
	}
}
