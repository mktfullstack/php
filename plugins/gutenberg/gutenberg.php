...
if ( ! function_exists( 'anesta_gutenberg_localize_script' ) ) {
	...
	function anesta_gutenberg_localize_script( $arr ) {
		...
		if ( 'inherit' == $arr['sidebar_position'] ) {
			if ( 'post' == $post_type ) {
				$arr['sidebar_position'] = anesta_get_theme_option( 'sidebar_position_single' );
				if ( 'inherit' == $arr['sidebar_position'] ) {
					$arr['sidebar_position'] = anesta_get_theme_option( 'sidebar_position_blog' );
				}
			} else if ( 'page' != $post_type && anesta_check_theme_option( 'sidebar_position_single_' . sanitize_title( $post_slug ) ) ) {
				$arr['sidebar_position'] = anesta_get_theme_option( 'sidebar_position_single_' . sanitize_title( $post_slug ) );
				if ( 'inherit' == $arr['sidebar_position'] && anesta_check_theme_option( 'sidebar_position_' . sanitize_title( $post_slug ) ) ) {
					$arr['sidebar_position'] = anesta_get_theme_option( 'sidebar_position_' . sanitize_title( $post_slug ) );
				}
			}
			if ( 'inherit' == $arr['sidebar_position'] ) {
				$arr['sidebar_position'] = anesta_get_theme_option( 'sidebar_position' );
			}
		}
		if ( 'inherit' == $arr['expand_content'] ) {
			if ( 'post' == $post_type ) {
				$arr['expand_content'] = anesta_get_theme_option( 'expand_content_single' );
				if ( 'inherit' == $arr['expand_content'] ) {
					$arr['expand_content'] = anesta_get_theme_option( 'expand_content_blog' );
				}
			} else if ( 'page' != $post_type && anesta_check_theme_option( 'expand_content_single_' . sanitize_title( $post_slug ) ) ) {
				$arr['expand_content'] = anesta_get_theme_option( 'expand_content_single_' . sanitize_title( $post_slug ) );
				if ( 'inherit' == $arr['expand_content'] && anesta_check_theme_option( 'expand_content_' . sanitize_title( $post_slug ) ) ) {
					$arr['expand_content'] = anesta_get_theme_option( 'expand_content_' . sanitize_title( $post_slug ) );
				}
			}
			if ( 'inherit' == $arr['expand_content'] ) {
				$arr['expand_content'] = anesta_get_theme_option( 'expand_content' );
			}
		}
		return $arr;
	}
...
if ( anesta_exists_gutenberg() ) {
	require_once anesta_get_file_dir( 'plugins/gutenberg/gutenberg-style.php' );
	require_once anesta_get_file_dir( 'plugins/gutenberg/gutenberg-fse.php' );
}
